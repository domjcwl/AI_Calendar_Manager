...
def list_events(days_ahead: int = 7) -> str:
    ...
    try:
        ...
        for e in events:
            ...
            # Handle all-day multi-day events (end date is exclusive)
            if "date" in e["start"]:
...
def _get_tasks_service():
    ...
    from calendar_auth import get_credentials  # expose creds from your auth module
...
# ===========================================================================
# BIRTHDAY TOOLS
# Google Calendar stores birthdays as all-day recurring yearly events.
# They live in a dedicated "Birthdays" calendar (read-only on most accounts),
# so we create them in the *primary* calendar with a "birthday" label instead,
# which is the standard workaround for full CRUD control.
# ===========================================================================

BIRTHDAY_TAG = "#birthday"   # appended to description so we can filter later
...
def list_birthdays(days_ahead: int = 365) -> str:
    ...
    Returns:
        Formatted list of upcoming birthdays.
    """
    try:
        service = get_calendar_service()
        now    = datetime.datetime.utcnow().isoformat() + "Z"
...
def update_birthday(
    ...
    Returns:
        Confirmation with updated event link.
    """
    try:
        service = get_calendar_service()
        event   = service.events().get(calendarId="primary", eventId=event_id).execute()
        ...
        if note is not None:
            ...
            # Keep the tag, replace user note
...
# ===========================================================================
# TASK TOOLS
# Uses the Google Tasks API (separate from Calendar Events).
# Tasks live in task lists; we target "@default" unless a list title is given.
# ===========================================================================
...
def add_task(
    ...
    try:
        ...
        if due_date:
            # Tasks API requires RFC 3339 UTC midnight
...
def update_task(
    ...
    try:
        ...
        updated = tasks_svc.tasks().update(
...
# ---------------------------------------------------------------------------
# Internal helper (not a @tool — used by task tools above)
# ---------------------------------------------------------------------------
...
def _resolve_task_list(tasks_svc, name_or_id: str) -> str:
    ...
    if name_or_id in ("@default", "") or name_or_id.startswith("MTAw"):
        return name_or_id  # already an ID or the default sentinel
